...
/**********************************************************************************************/
// PPB PORE force code begins
/*static real pore_energy(real r_eff, real par6 real par12)
 {
 real E;
 E = -1*par6*pow(r_eff,-6) + par12*pow(r_eff,-12);

 E = -1*ir->porePar6*pow(r_eff,-6) + ir->porePar12*pow(r_eff,-12);
 return E;
 }
 */
...
//PPB Pore force code ends
/*************************************************************************************************/
static void
do_update_md (int start, int nrend, double dt, t_grp_tcstat *tcstat,
	      double nh_vxi[], gmx_bool bNEMD, t_grp_acc *gstat, rvec accel[],
	      ivec nFreeze[], real invmass[], unsigned short ptype[],
	      unsigned short cFREEZE[], unsigned short cACC[],
	      unsigned short cTC[], rvec x[], rvec xprime[], rvec v[], rvec f[],
	      matrix M, gmx_bool bNH, gmx_bool bPR)
{
  double imass, w_dt;
  int gf = 0, ga = 0, gt = 0;
  rvec vrel;
  ...
  else if (cFREEZE != NULL || nFreeze[0][XX] || nFreeze[0][YY] || nFreeze[0][ZZ]
      || bNEMD)
    {
...
static void
do_update_vv_vel (int start, int nrend, double dt, t_grp_tcstat *tcstat,
		  t_grp_acc *gstat, rvec accel[], ivec nFreeze[],
		  real invmass[], unsigned short ptype[],
		  unsigned short cFREEZE[], unsigned short cACC[], rvec v[],
...
static void
do_update_vv_pos (int start, int nrend, double dt, t_grp_tcstat *tcstat,
		  t_grp_acc *gstat, rvec accel[], ivec nFreeze[],
		  real invmass[], unsigned short ptype[],
		  unsigned short cFREEZE[], rvec x[], rvec xprime[], rvec v[],
		  rvec f[], gmx_bool bExtended, real veta, real alpha)
{
  double imass, w_dt;
  int gf = 0;
...
static void
do_update_visc (int start, int nrend, double dt, t_grp_tcstat *tcstat,
		double nh_vxi[], real invmass[], unsigned short ptype[],
		unsigned short cTC[], rvec x[], rvec xprime[], rvec v[],
		rvec f[], matrix M, matrix box, real cos_accel, real vcos,
 ...
 */
static void
init_multiple_gaussrand (gmx_stochd_t *sd)
{
  int ngr, i;
  unsigned int *seed;
  ...
  if (ngr != gmx_omp_nthreads_get (emntUpdate))
    {
      gmx_incons(
	  "The number of Gaussian number generators should be equal to gmx_omp_nthreads_get(emntUpdate)");
    }
...
static gmx_stochd_t *
init_stochd (FILE *fplog, t_inputrec *ir, int nthreads)
{
  gmx_stochd_t *sd;
  gmx_sd_const_t *sdc;
  int ngtc, n, th;
      ...
      for (n = 0; n < ngtc; n++)
	{
	  ...
	  if (sdc[n].gdt >= 0.05)
	  ...
	  else
	    {
	      ...
	      sdc[n].b = y * y * y * y
		  * (1 / 3.0 + y * (1 / 3.0 + y * (17 / 90.0 + y * 7 / 9.0)));
	      sdc[n].c = y * y * y
		  * (2 / 3.0
		      + y
			  * (-1 / 2.0
...
void
get_stochd_state (gmx_update_t upd, t_state *state)
{
...
void
set_stochd_state (gmx_update_t upd, t_state *state)
{
  gmx_stochd_t *sd;
  int i;
...
gmx_update_t
init_update (FILE *fplog, t_inputrec *ir)
{
  t_gmx_update *upd;
  ...
  if (ir->eI
      == eiBD|| EI_SD(ir->eI) || ir->etc == etcVRESCALE || ETC_ANDERSEN(ir->etc))
    {
...
static void
do_update_sd1 (gmx_stochd_t *sd, gmx_rng_t gaussrand, int start, int nrend,
	       double dt, rvec accel[], ivec nFreeze[], real invmass[],
	       unsigned short ptype[], unsigned short cFREEZE[],
	       unsigned short cACC[], unsigned short cTC[], rvec x[],
	       rvec xprime[], rvec v[], rvec f[], rvec sd_X[], int ngtc,
	       real tau_t[], real ref_t[])
{
  gmx_sd_const_t *sdc;
  gmx_sd_sigma_t *sig;
  real kT;
      ...
      for (d = 0; d < DIM; d++)
	{
	  if ((ptype[n] != eptVSite) && (ptype[n] != eptShell)
	      && !nFreeze[gf][d])
	    {
		  ...
		  + (invmass[n] * f[n][d] + accel[ga][d]) * tau_t[gt]
		      * (1 - sdc[gt].em) + sd_V;
...
static void
check_sd2_work_data_allocation (gmx_stochd_t *sd, int nrend)
{
...
static void
do_update_sd2_Tconsts (gmx_stochd_t *sd, int ngtc, const real tau_t[],
		       const real ref_t[])
{
...
static void
do_update_sd2 (gmx_stochd_t *sd, gmx_rng_t gaussrand, gmx_bool bInitStep,
	       int start, int nrend, rvec accel[], ivec nFreeze[],
	       real invmass[], unsigned short ptype[], unsigned short cFREEZE[],
	       unsigned short cACC[], unsigned short cTC[], rvec x[],
	       rvec xprime[], rvec v[], rvec f[], rvec sd_X[],
	       const real tau_t[], gmx_bool bFirstHalf)
{
  gmx_sd_const_t *sdc;
  gmx_sd_sigma_t *sig;
      ...
      for (d = 0; d < DIM; d++)
	{
	  ...
	  if ((ptype[n] != eptVSite) && (ptype[n] != eptShell)
	      ...
	      else
		{
		   ...
		   */
		  v[n][d] = (xprime[n][d] - x[n][d])
		      / (tau_t[gt] * (sdc[gt].eph - sdc[gt].emh));
...
static void
do_update_bd_Tconsts (double dt, real friction_coefficient, int ngtc,
		      const real ref_t[], real *rf)
{
  /* This is separated from the update below, because it is single threaded */
  int gt;
...
static void
do_update_bd (int start, int nrend, double dt, ivec nFreeze[], real invmass[],
	      unsigned short ptype[], unsigned short cFREEZE[],
	      unsigned short cTC[], rvec x[], rvec xprime[], rvec v[], rvec f[],
	      real friction_coefficient, real *rf, gmx_rng_t gaussrand)
{
...
static void
dump_it_all (FILE *fp, const char *title, int natoms, rvec x[], rvec xp[],
	     rvec v[], rvec f[])
{
#ifdef DEBUG
  if (fp)
...
static void
calc_ke_part_normal (rvec v[], t_grpopts *opts, t_mdatoms *md,
  ...
  for (thread = 0; thread < nthread; thread++)
      ...
      for (n = start_t; n < end_t; n++)
	{
	  ...
	  if (md->nMassPerturbed && md->bPerturbed[n])
	    {
	      *dekindl_sum += 0.5 * (md->massB[n] - md->massA[n])
		  * iprod (v_corrt, v_corrt);
	    }
	}
    }
...
static void
calc_ke_part_visc (matrix box, rvec x[], rvec v[], t_grpopts *opts,
		   t_mdatoms *md, gmx_ekindata_t *ekind, t_nrnb *nrnb,
		   gmx_bool bEkinAveVel, gmx_bool bSaveEkinOld)
{
  ...
  for (n = start; n < start + homenr; n++)
      ...
      if (md->nPerturbed && md->bPerturbed[n])
	{
	   ...
	   */
	  dekindl -= 0.5 * (md->massB[n] - md->massA[n])
	      * iprod (v_corrt, v_corrt);
	}
    }
...
void
calc_ke_part (t_state *state, t_grpopts *opts, t_mdatoms *md,
	      gmx_ekindata_t *ekind, t_nrnb *nrnb, gmx_bool bEkinAveVel,
	      gmx_bool bSaveEkinOld)
{
...
void
restore_ekinstate_from_state (t_commrec *cr, gmx_ekindata_t *ekind,
...
void
set_deform_reference_box (gmx_update_t upd, gmx_large_int_t step, matrix box)
{
...
static void
deform (gmx_update_t upd, int start, int homenr, rvec x[], matrix box,
	matrix *scale_tot, const t_inputrec *ir, gmx_large_int_t step)
{
  ...
  for (i = 0; i < DIM; i++)
      ...
      for (j = 0; j < DIM; j++)
	{
	  if (ir->deform[i][j] != 0)
	    {
	      bnew[i][j] = upd->deformref_box[i][j]
		  + elapsed_time * ir->deform[i][j];
	    }
	}
    }
  ...
  for (i = start; i < start + homenr; i++)
    {
      x[i][XX] = mu[XX][XX] * x[i][XX] + mu[YY][XX] * x[i][YY]
	  + mu[ZZ][XX] * x[i][ZZ];
...
void
update_tcouple (FILE *fplog, gmx_large_int_t step, t_inputrec *inputrec,
		t_state *state, gmx_ekindata_t *ekind, gmx_wallcycle_t wcycle,
		gmx_update_t upd, t_extmass *MassQ, t_mdatoms *md)
  ...
  if (bTCouple)
	...
	case etcNOSEHOOVER:
	  nosehoover_tcoupl (&(inputrec->opts), ekind, dttc,
			     state->nosehoover_xi, state->nosehoover_vxi,
			     MassQ);
	  break;
	case etcVRESCALE:
	  vrescale_tcoupl (inputrec, ekind, dttc, state->therm_integral,
			   upd->sd->gaussrand[0]);
	  break;
	}
      ...
      if (EI_VV(inputrec->eI))
	{
	  rescale_velocities (ekind, md, md->start, md->start + md->homenr,
			      state->v);
	}
    }
  else
...
void
update_pcouple (FILE *fplog, gmx_large_int_t step, t_inputrec *inputrec,
		t_state *state, matrix pcoupl_mu, matrix M,
		gmx_wallcycle_t wcycle, gmx_update_t upd, gmx_bool bInitStep)
{
  ...
  if (inputrec->epc != epcNO
      && (!(IR_NPT_TROTTER(inputrec) || IR_NPH_TROTTER(inputrec))))
    {
      /* We should only couple after a step where energies were determined */
      bPCouple =
	  (inputrec->nstpcouple == 1
  ...
  if (bPCouple)
	...
	case (epcBERENDSEN):
	  if (!bInitStep)
	    {
	      berendsen_pcoupl (fplog, step, inputrec, dtpc, state->pres_prev,
				state->box, pcoupl_mu);
	    }
	  break;
	case (epcPARRINELLORAHMAN):
	  parrinellorahman_pcoupl (fplog, step, inputrec, dtpc,
				   state->pres_prev, state->box, state->box_rel,
				   state->boxv, M, pcoupl_mu, bInitStep);
	  break;
	default:
	  break;
	}
...
static rvec *
get_xprime (const t_state *state, gmx_update_t upd)
{
...
static void
combine_forces (gmx_update_t upd, int nstcalclr, gmx_constr_t constr,
		t_inputrec *ir, t_mdatoms *md, t_idef *idef, t_commrec *cr,
		gmx_large_int_t step, t_state *state, gmx_bool bMolPBC,
		int start, int nrend, rvec f[], rvec f_lr[],
		tensor *vir_lr_constr, t_nrnb *nrnb)
{
  int i, d;
  ...
  if (constr != NULL && vir_lr_constr != NULL
      ...
      for (i = md->start; i < md->start + md->homenr; i++)
	{
	  ...
	  for (d = 0; d < DIM; d++)
	    {
	      if ((md->ptype[i] != eptVSite) && (md->ptype[i] != eptShell)
      ...
      constrain (NULL, FALSE, FALSE, constr, idef, ir, NULL, cr, step, 0, md,
		 state->x, xp, xp, bMolPBC, state->box,
		 state->lambda[efptBONDED], NULL,
		 NULL,
		 vir_lr_constr, nrnb, econqCoord, ir->epc == epcMTTK,
		 state->veta, state->veta);
    }
...
void
update_constraints (FILE *fplog, gmx_large_int_t step, real *dvdlambda, /* the contribution to be added to the bonded interactions */
		    t_inputrec *inputrec, /* input record and box stuff	*/
		    gmx_ekindata_t *ekind, t_mdatoms *md, t_state *state,
		    gmx_bool bMolPBC, t_graph *graph, rvec force[], /* forces on home particles */
		    t_idef *idef, tensor vir_part, tensor vir, /* tensors for virial and ekin, needed for computing */
		    t_commrec *cr, t_nrnb *nrnb, gmx_wallcycle_t wcycle,
		    gmx_update_t upd, gmx_constr_t constr, gmx_bool bInitStep,
		    gmx_bool bFirstHalf, gmx_bool bCalcVir, real vetanew)
{
  gmx_bool bExtended, bLastStep, bLog = FALSE, bEner = FALSE, bDoConstr =
  FALSE;
  double dt;
  real dt_1;
  ...
  if (bDoConstr)
      ...
      if (EI_VV(inputrec->eI) && bFirstHalf)
	{
	  constrain (NULL, bLog, bEner, constr, idef, inputrec, ekind, cr, step,
		     1, md, state->x, state->v, state->v, bMolPBC, state->box,
		     state->lambda[efptBONDED], dvdlambda,
		     NULL,
		     bCalcVir ? &vir_con : NULL, nrnb, econqVeloc,
		     inputrec->epc == epcMTTK, state->veta, vetanew);
	}
      else
	{
	  constrain (NULL, bLog, bEner, constr, idef, inputrec, ekind, cr, step,
		     1, md, state->x, xprime, NULL, bMolPBC, state->box,
		     state->lambda[efptBONDED], dvdlambda, state->v,
		     bCalcVir ? &vir_con : NULL, nrnb, econqCoord,
      ...
      dump_it_all (fplog, "After Shake", state->natoms, state->x, xprime,
		   state->v, force);
...
#pragma omp parallel for num_threads(nth) schedule(static)
      for (th = 0; th < nth; th++)
	{
	  ...
	  FALSE,
			 start_th, end_th, inputrec->opts.acc,
			 inputrec->opts.nFreeze, md->invmass, md->ptype,
			 md->cFREEZE, md->cACC, md->cTC, state->x, xprime,
			 state->v, force, state->sd_X, inputrec->opts.tau_t,
			 FALSE);
	}
      ...
      if (bDoConstr)
	{
	  ...
	  constrain (NULL, bLog, bEner, constr, idef, inputrec, NULL, cr, step,
		     ...
		     state->lambda[efptBONDED], dvdlambda,
		     NULL,
		     NULL, nrnb, econqCoord, FALSE, 0, 0);
	  wallcycle_stop (wcycle, ewcCONSTR);
	}
    }
...
#pragma omp parallel for num_threads(gmx_omp_nthreads_get(emntUpdate)) schedule(static)
      ...
      dump_it_all (fplog, "After unshift", state->natoms, state->x, upd->xp,
		   state->v, force);
    }
...
void
update_box (FILE *fplog, gmx_large_int_t step, t_inputrec *inputrec, /* input record and box stuff	*/
	    t_mdatoms *md, t_state *state, t_graph *graph, rvec force[], /* forces on home particles */
	    matrix *scale_tot, matrix pcoupl_mu, t_nrnb *nrnb,
	    gmx_wallcycle_t wcycle, gmx_update_t upd, gmx_bool bInitStep,
	    gmx_bool bFirstHalf)
{
  ...
  bExtended = (inputrec->etc == etcNOSEHOOVER)
      || (inputrec->epc == epcPARRINELLORAHMAN) || (inputrec->epc == epcMTTK);
  ...
  switch (inputrec->epc)
    ...
    case (epcNO):
      break;
    case (epcBERENDSEN):
      berendsen_pscale (inputrec, pcoupl_mu, state->box, state->box_rel, start,
			homenr, state->x, md->cFREEZE, nrnb);
      break;
    case (epcPARRINELLORAHMAN):
  ...
  if (DEFORM(*inputrec))
    {
      deform (upd, start, homenr, state->x, state->box, scale_tot, inputrec,
	      step);
    }
  where();
  dump_it_all (fplog, "After update", state->natoms, state->x, upd->xp,
	       state->v, force);
}

void
update_coords (FILE *fplog, gmx_large_int_t step, t_inputrec *inputrec, /* input record and box stuff	*/
	       t_mdatoms *md, t_state *state, gmx_bool bMolPBC, rvec *f, /* forces on home particles */
	       gmx_bool bDoLR, rvec *f_lr, tensor *vir_lr_constr, t_fcdata *fcd,
	       gmx_ekindata_t *ekind, matrix M, gmx_wallcycle_t wcycle,
	       gmx_update_t upd, gmx_bool bInitStep, int UpdatePart,
	       t_commrec *cr, /* these shouldn't be here -- need to think about it */
	       t_nrnb *nrnb, gmx_constr_t constr, t_idef *idef, gmx_bool bPORE)
{
  // printf("\n\nupdate_coords() called \n");  //PPB
  ...
  int *icom = NULL;
  tensor vir_con;
  rvec *vcom, *xcom, *vall, *xall, *xin, *vin, *forcein, *fall, *xpall,
      *xprimein, *xprime;
  int nth, th;
  ...
  //PPB  Put the force here

  //while(0){
  //  if (bPORE)
  //    do_pore_force (fplog, step, cr, inputrec, md, state->x, f);
  //  }
  if (bDoLR && inputrec->nstcalclr > 1 && !EI_VV(inputrec->eI)) /* get this working with VV? */
    {
      ...
      combine_forces (upd, inputrec->nstcalclr, constr, inputrec, md, idef, cr,
		      step, state, bMolPBC, start, nrend, f, f_lr,
		      vir_lr_constr, nrnb);
      force = f_lr;
    }
  else
  ...
  where();
  dump_it_all (fplog, "Before update", state->natoms, state->x, xprime,
	       state->v, force);
  ...
  if (inputrec->eI == eiSD2)
    {
      ...
      do_update_sd2_Tconsts (upd->sd, inputrec->opts.ngtc, inputrec->opts.tau_t,
			     inputrec->opts.ref_t);
    }
  if (inputrec->eI == eiBD)
    {
      do_update_bd_Tconsts (dt, inputrec->bd_fric, inputrec->opts.ngtc,
			    inputrec->opts.ref_t, upd->sd->bd_rf);
    }
...
#pragma omp parallel for num_threads(nth) schedule(static) private(alpha)
  for (th = 0; th < nth; th++)
	...
	case (eiMD):
	  if (ekind->cosacc.cos_accel == 0)
	    {
	      do_update_md (start_th, end_th, dt, ekind->tcstat,
			    state->nosehoover_vxi, ekind->bNEMD, ekind->grpstat,
			    inputrec->opts.acc, inputrec->opts.nFreeze,
			    md->invmass, md->ptype, md->cFREEZE, md->cACC,
			    md->cTC, state->x, xprime, state->v, force, M, bNH,
			    bPR);
	    }
	  else
	    {
	      do_update_visc (start_th, end_th, dt, ekind->tcstat,
			      state->nosehoover_vxi, md->invmass, md->ptype,
			      md->cTC, state->x, xprime, state->v, force, M,
			      state->box, ekind->cosacc.cos_accel,
			      ekind->cosacc.vcos, bNH, bPR);
	    }
	  break;
	case (eiSD1):
	  do_update_sd1 (upd->sd, upd->sd->gaussrand[th], start_th, end_th, dt,
			 inputrec->opts.acc, inputrec->opts.nFreeze,
			 md->invmass, md->ptype, md->cFREEZE, md->cACC, md->cTC,
	...
	case (eiSD2):
	   ...
	   * is needed
	   */
	  do_update_sd2 (upd->sd, upd->sd->gaussrand[th], bInitStep, start_th,
			 end_th, inputrec->opts.acc, inputrec->opts.nFreeze,
			 md->invmass, md->ptype, md->cFREEZE, md->cACC, md->cTC,
			 state->x, xprime, state->v, force, state->sd_X,
			 inputrec->opts.tau_t,
			 TRUE);
	  break;
	case (eiBD):
	  do_update_bd (start_th, end_th, dt, inputrec->opts.nFreeze,
			md->invmass, md->ptype, md->cFREEZE, md->cTC, state->x,
			xprime, state->v, force, inputrec->bd_fric,
	...
	case (eiVVAK):
	  ...
	  switch (UpdatePart)
	    ...
	    case etrtVELOCITY1:
	    case etrtVELOCITY2:
	      do_update_vv_vel (start_th, end_th, dt, ekind->tcstat,
				ekind->grpstat, inputrec->opts.acc,
				inputrec->opts.nFreeze, md->invmass, md->ptype,
				md->cFREEZE, md->cACC, state->v, force,
				(bNH || bPR), state->veta, alpha);
	      break;
	    case etrtPOSITION:
	      do_update_vv_pos (start_th, end_th, dt, ekind->tcstat,
				ekind->grpstat, inputrec->opts.acc,
				inputrec->opts.nFreeze, md->invmass, md->ptype,
				md->cFREEZE, state->x, xprime, state->v, force,
				(bNH || bPR), state->veta, alpha);
	      break;
	    }
	  break;
...
void
correct_ekin (FILE *log, int start, int end, rvec v[], rvec vcm, real mass[],
...
extern gmx_bool
update_randomize_velocities (t_inputrec *ir, gmx_large_int_t step,
			     t_mdatoms *md, t_state *state, gmx_update_t upd,
			     t_idef *idef, gmx_constr_t constr,
			     gmx_bool bIsDomainDecomposition)
{
  ...
  if (ir->etc == etcANDERSEN && constr && bIsDomainDecomposition)
    {
      gmx_fatal (
	  FARGS,
	  "Normal Andersen is currently not supported with constraints and domain decomposition. Please consider the massive Andersen thermostat.");
    }
  ...
  if ((ir->etc == etcANDERSEN) || do_per_step (step, (int) (1.0 / rate)))
      ...
      if (upd->randatom_list_init == FALSE)
	{
      ...
      andersen_tcoupl (ir, md, state, upd->sd->gaussrand[0], rate,
		       ...
		       constr ? get_sblock (constr) : NULL, upd->randatom,
		       upd->randatom_list, upd->sd->randomize_group,
		       upd->sd->boltzfac);
      return TRUE;
    }
  return FALSE;
