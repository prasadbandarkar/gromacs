...
static void
do_pore_force (FILE *fplog, gmx_large_int_t step, t_commrec *cr, t_inputrec *ir,
	       t_mdatoms *md, rvec xi[], rvec fi[],  gmx_enerdata_t *enerd )
{

  int start, homenr, lindex;
  real x, y, z;  // new coordinates with centre at 0 0 0
  real dir[3];
  ...
  real h1 = ir->poreh1;
  real h2 = ir->poreh2;
  real alpha = ir->poreAlpha;

  real sinA = sin(alpha*M_PI/180);
  real cosA = cos(alpha*M_PI/180);
  real tanA = sinA/cosA;

  int mod = (step % (ir->nstenergy));

  start = md->start;
  homenr = md->homenr + start;
  int i;
  ...
  for (i = start; i < homenr; i++)
    {
	  ...
	  if (rho!=0.0){
	      r_eff = R - rho;
	      f_mag = -1.0 * 6.0 * ir->porePar6 * pow (r_eff, -7.0)
	      + 12.0 * ir->porePar12 * pow (r_eff, -13.0);
	      fi[i][0] += -1.0 * f_mag * x / rho;
	      fi[i][1] += -1.0 * f_mag * y / rho;
	      if (mod == 0)
		{
		  E += -1.0 * ir->porePar6 * pow (r_eff, -6.0)
		  + ir->porePar12 * pow (r_eff, -12.0);
		};
	  }
      }
      else if( (z>h1) && (tan((z-h1)/(R-rho)) < tanA) ){ //Region2
	  if (rho == 0.0)
	    {
	      r_eff = sqrt (pow (R, 2) + pow ((z - h1), 2));
	      f_mag = -1.0 * 6.0 * ir->porePar6 * pow (r_eff, -7.0)
	      + 12.0 * ir->porePar12 * pow (r_eff, -13.0);
	      fi[i][2] += f_mag;
	      if (mod == 0)
	  ...
	  else
	    {
	      ...
	      + 12.0 * ir->porePar12 * pow (r_eff, -13.0);
	  					...
	  					* (dir[0] * (1.0 - R / rho + R * x * x * pow (rho, -3.0))
	  					    + dir[1] * R * x * y * pow (rho, -3.0));
	      fi[i][1] += (f_mag / r_eff)
	  					* (dir[0] * R * x * y * pow (rho, -3.0)
	      + dir[1]
		    * (1.0 - R / rho + R * y * y * pow (rho, -3.0)));
	      fi[i][2] += (f_mag / r_eff) * (dir[2]);
      ...
      else if((z>h1) && (z<=h2)){ // Region 3
	  r_eff = (R-rho)*cosA + (z-h1)*sinA;
	  f_mag = -1.0 * 6.0 * ir->porePar6 * pow (r_eff, -7.0)
	  	      + 12.0 * ir->porePar12 * pow (r_eff, -13.0);
	  fi[i][0]+=f_mag*(-1.0)*x*cosA/rho;
	  fi[i][1]+=f_mag*(-1.0)*y*cosA/rho;
	  fi[i][2]+=f_mag*cosA;
	  if(mod ==0){
	      E += -1.0 * ir->porePar6 * pow (r_eff, -6.0)
	      		  + ir->porePar12 * pow (r_eff, -12.0);
      ...
      else if ((z>h2)&&(tan((z-h2)/(R-rho +(h2-h1)*tanA)))<tanA){// Region 4
	  r_eff = (R-rho)*cosA + (z-h1)*sinA;
	  	  f_mag = -1.0 * 6.0 * ir->porePar6 * pow (r_eff, -7.0)
	  	  	      + 12.0 * ir->porePar12 * pow (r_eff, -13.0);
	  	  fi[i][0]+=f_mag*(-1.0)*x*cosA/rho;
      ...
      else if((z>h2)&&(rho<(R+(h2-h1)*tanA))){ //Region 5
	  real R2 =  R+(h2-h1)*tanA;
	  if (rho == 0.0)
	    {
	      r_eff = sqrt (pow (R2, 2) + pow ((z - h2), 2));
	      f_mag = -1.0 * 6.0 * ir->porePar6 * pow (r_eff, -7.0)
	      + 12.0 * ir->porePar12 * pow (r_eff, -13.0);
	      fi[i][2] += f_mag;
	      if (mod == 0)
	  ...
	  else
	    {
	      ...
	      + 12.0 * ir->porePar12 * pow (r_eff, -13.0);
	               ...
	               * (dir[0] * (1.0 - R2 / rho + R2 * x * x * pow (rho, -3.0))
	                + dir[1] * R2 * x * y * pow (rho, -3.0));
	      fi[i][1] += (f_mag / r_eff)
	  	  	* (dir[0] * R2 * x * y * pow (rho, -3.0)
	                    +  dir[1]* (1.0 - R2 / rho + R2 * y * y * pow (rho, -3.0)));
	      fi[i][2] += (f_mag / r_eff) * (dir[2]);
      ...
      else if((z>h2)&&(rho>=(R+(h2-h1)*tanA))){ //Region 6
	  r_eff = z - h2 ;
	  f_mag = -1.0 * 6.0 * ir->porePar6 * pow (r_eff, -7.0)
	  + 12.0 * ir->porePar12 * pow (r_eff, -13.0);
	  ...
	  gmx_fatal (
	        FARGS,
	       "Atom found inside the pore wall! Use the cpt file to run the system again!\n");
      }
  ...
  if (mod == 0)
    {
      MPI_Reduce (&E, &globalE, 1, GMX_MPI_REAL, MPI_SUM, MASTERRANK(cr),
		  cr->mpi_comm_mygroup); //Check this line the last parameter needs to be checked
      if (MASTER(cr))
	{
	  double poretime = step * (ir->delta_t);
	  FILE *fp;
	  real LJ;
	  LJ = enerd->term[F_EPOT];
	  enerd->term[F_EPOT]  += globalE;
	  fp = fopen ("pore-wall-energy.txt", "a");
	  fprintf (fp, "%lf\t%E\t%E\n", poretime,LJ, globalE);
	  fclose (fp);
	  globalE = 0.0;
	  LJ = 0.0;
...
void do_force(FILE *fplog, t_commrec *cr,
	      ...
	      double t, FILE *field, gmx_edsam_t ed,
	      gmx_bool bBornRadii,
	      int flags,gmx_bool bPORE) //PPB_2
{
  ...
  if (bPORE)
    {
      do_pore_force (fplog, step, cr, inputrec, mdatoms , x, f, enerd ); //PPB_2
    }
