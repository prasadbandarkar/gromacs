...
double do_md(FILE *fplog, t_commrec *cr, int nfile, const t_filenm fnm[],
             ...
             unsigned long Flags,
             gmx_runtime_t *runtime, gmx_bool bPORE) // PPB_2
{
    gmx_mdoutf_t   *outf;
    ...
    //gmx_bool        bPORE = FALSE; // PPB PORE flag
    ...
    bPORE    = (Flags & MD_PORE);// PPB pore flag is set

    //PPB pore
    if (MASTER(cr)) {
     if (bPORE && ( (ir->poreX < ir->poreR ) || (ir->poreY < ir-> poreR) ) )
       gmx_fatal(FARGS,"PORE is on. The pore goes outside the box. This might be an issue incase of periodic boundary conditions");
     if (bPORE && ir->poreR <=0)
        gmx_fatal(FARGS,"PORE is on. The radius of the pore must be positive");
    }
 //PPB code ends
    ...
    while (!bLastStep || (bRerunMD && bNotLastFrame))
    {
        ...
        else
        {
            ...
            do_force(fplog, cr, ir, step, nrnb, wcycle, top, top_global, groups,
                     ...
                     (bNS ? GMX_FORCE_NS : 0) | force_flags, bPORE); // PPB_2
        }
        ...
        /*  ############### START FIRST UPDATE HALF-STEP FOR VV METHODS############### */
        {
            ...
            update_coords(fplog, step, ir, mdatoms, state, fr->bMolPBC,
                          ...
                          cr, nrnb, constr, &top->idef, bPORE); // PPB
        ...
        while (bFirstIterate || iterate.bIterationActive)
        {
            ...
            if (!(bRerunMD && !rerun_fr.bV && !bForceUpdate))
            {
                ...
                update_coords(fplog, step, ir, mdatoms, state, fr->bMolPBC, f,
                              bUpdateDoLR, fr->f_twin, bCalcVir ? &fr->vir_twin_constr : NULL, fcd,
                              ekind, M, wcycle, upd, bInitStep, etrtPOSITION, cr, nrnb, constr, &top->idef, bPORE ); //PPB
